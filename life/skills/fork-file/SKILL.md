...
1. Use `mcp__imessages__imessages_conversations` to identify the user's self-thread, then `mcp__imessages__imessages_messages` to find the most recent message with an image attachment.
...
5. Group items by likely storage location and present a confirmation table:
...
2. For any item without a specified location, group by likely location and ask for confirmation.
...
1. Use `mcp__imessages__imessages_conversations` and `mcp__imessages__imessages_messages` to find the most recent image in the self-thread.
...
- **NEVER write to either CSV without explicit user approval.** Always present a complete review table and wait for "looks good", "save", "yes", or similar. If corrections are made, update and show the table again.
...
- For ambiguous food queries (e.g., "do I have chicken?"), check all grocery locations and report all matches.
